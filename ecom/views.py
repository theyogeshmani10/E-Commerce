...
def home_view(request):
    ...
    #for recommending most selling products(3) to the user
    pproducts=(models.Product.objects.filter(id__in =models.Orders.objects.values('product').annotate(total=Count('product_id'),).order_by('total').values_list('product_id')))[:3]
    ...
    return render(request,'ecom/index.html',{'products':products,'product_count_in_cart':product_count_in_cart,'pproducts':pproducts})
...
def cart_view(request):
    ...
    corr = pickle.load(open('corr.sav','rb'))
    ...
    if 'product_ids' in request.COOKIES:
        ...
        if product_ids != "":
            ...
            for p in products:
                total=total+p.price
                correlation_product_ID = corr[p.pk-1]
                for i in range(0, correlation_product_ID.length):
                    if(correlation_product_ID[i]>.80):
                        Recommend.append(i+1)
    
    print(Recommend)
    pproducts = models.Product.objects.filter(id__in=Recommend)

    return render(request,'ecom/cart.html',{'products':products,'total':total,'product_count_in_cart':product_count_in_cart,'pproducts':pproducts})
